---
title: Course welcome
description: Meet your instructor and course overview
icon: "hand-wave"
---

## Welcome to learn Python for AI

I'm Dave Ebbelaar, and I'll be your guide on this journey into Python and AI development.

I started learning Python in 2013 when I began my bachelor's in Artificial Intelligence. Today, I have over a decade of Python experience and have taught this language to over 30,000 students through my courses and YouTube channel.

But it didn't start smoothly. I actually failed my first Python class.

That experience taught me something crucial: there are two parts to learning Python. First, there's the syntax - how to write the code. Python is easy here; you can almost write it like plain English, and AI assistants can help with any syntax questions.

The second part is what most courses skip: your workflow. What tools do you use? How do you set them up? How do you start projects, save files, debug code, and structure your work? These skills are just as important as the code itself.

When you understand your workflow, your learning accelerates dramatically. Combine that with AI tools, and you'll progress faster than ever before.

This is the course I wish I had 10 years ago. After running an AI software company for five years and completing over 50 projects across industries, I'm bringing all that experience to help you learn effectively.

## What you'll learn

<CardGroup cols={3}>
  <Card title="Python fundamentals" icon="code">
    Variables, functions, and data structures
  </Card>
  <Card title="Professional workflow" icon="laptop">
    Tools, debugging, and project structure
  </Card>
  <Card title="AI foundations" icon="robot">
    Essential Python skills for AI development
  </Card>
</CardGroup>

## Ready to start?

<Card
  title="Continue to why Python"
  icon="arrow-right"
  href="/introduction/why-python"
>
  Discover why Python dominates the AI landscape
</Card>